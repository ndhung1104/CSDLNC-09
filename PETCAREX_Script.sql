...
CREATE TRIGGER TRG_APPOINTMENT_VET_ONLY
ON APPOINTMENT
AFTER INSERT, UPDATE
AS
BEGIN
	...
	IF EXISTS (
		SELECT 1
		FROM inserted i
		JOIN EMPLOYEE e ON i.VET_ID = e.EMPLOYEE_ID
		WHERE i.VET_ID IS NOT NULL AND e.EMPLOYEE_POSITION <> 'VET'
	)
	BEGIN
		RAISERROR ('VET_ID must reference a vet.', 16, 1);
		ROLLBACK TRANSACTION;
		RETURN;
	END
END
...
CREATE TABLE VET_SCHEDULE (
	VET_SCHEDULE_ID INT IDENTITY (1, 1) PRIMARY KEY,
	VET_ID INT NOT NULL,
	BRANCH_ID INT NOT NULL,
...
ALTER TABLE VET_SCHEDULE ADD
	CONSTRAINT FK_VETSCHEDULE_VET FOREIGN KEY (VET_ID) REFERENCES EMPLOYEE (EMPLOYEE_ID),
	CONSTRAINT FK_VETSCHEDULE_BRANCH FOREIGN KEY (BRANCH_ID) REFERENCES BRANCH (BRANCH_ID),
	CONSTRAINT CK_VETSCHEDULE_DOW CHECK (DAY_OF_WEEK BETWEEN 1 AND 7),
	CONSTRAINT CK_VETSCHEDULE_TIME CHECK (START_TIME < END_TIME)
GO
...
ALTER TABLE APPOINTMENT ADD
	...
	CONSTRAINT FK_APPOINTMENT_VET FOREIGN KEY (VET_ID) REFERENCES EMPLOYEE (EMPLOYEE_ID),
...
ALTER TABLE VACCINE ADD
	CONSTRAINT FK_VACCINE_PRODUCT FOREIGN KEY (VACCINE_ID) REFERENCES PRODUCT (PRODUCT_ID),
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IX_PET_CUSTOMER_ID'
      AND object_id = OBJECT_ID('dbo.PET')
)
    CREATE NONCLUSTERED INDEX IX_PET_CUSTOMER_ID
    ON PET (CUSTOMER_ID);
GO
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IX_RECEIPT_DETAIL_RECEIPT_ID'
      AND object_id = OBJECT_ID('dbo.RECEIPT_DETAIL')
)
    CREATE NONCLUSTERED INDEX IX_RECEIPT_DETAIL_RECEIPT_ID
    ON RECEIPT_DETAIL (RECEIPT_ID);
GO

IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IX_RECEIPT_BRANCH_DATE'
      AND object_id = OBJECT_ID('dbo.RECEIPT')
)
    CREATE NONCLUSTERED INDEX IX_RECEIPT_BRANCH_DATE
    ON RECEIPT (BRANCH_ID, RECEIPT_CREATED_DATE);
GO
